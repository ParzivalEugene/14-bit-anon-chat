    ...
    Room.find({participans: userID}, (err, res) => {
      if (err) {
        ...
        if (res.length > 0) {
          ...
          tg.sendMessage(userID, `[Ваш собеседник](tg://user?id=${partnerID})`, { parse_mode: 'MarkdownV2' })
            .catch(err => this.#errorWhenRoomActive(err, userID))
          tg.sendMessage(partnerID, `[Ваш собеседник](tg://user?id=${userID})`, { parse_mode: 'MarkdownV2' })
            .catch(err => this.#errorWhenRoomActive(err, userID))
    ...
    Room.find({participans: userID}, (err, res) => {
      if (err) {
        ...
        if (res.length > 0) {
          ...
          let index = participans.indexOf(userID)
          let partnerID = participans[index == 1 ? 0 : 1]
          tg.sendMessage(partnerID, text.USER_SHARE_USER,
          Markup.inlineKeyboard([[Markup.button.callback('Поделиться', 'shareUser')],]))
        }
      }
    })
  }
                ...
                .then(url => {
                  ...
                  tg.sendMessage(partnerID, text.USER_SEND_PHOTO.WARNING_1,
                    Markup.inlineKeyboard([
                      [Markup.button.callback('Показать фото', 'openPhoto-'+String(photoName))],
                    ])
                  ).catch(err => this.#errorWhenRoomActive(err, userID))
              ...
              tg.getFileLink(data)
                .then(url => {
                  ...
                  tg.sendMessage(partnerID, text.USER_SEND_VIDEO.WARNING_1,
                    Markup.inlineKeyboard([
                      [Markup.button.callback('Показать видео', 'openVideo-'+String(photoName))],
                    ])
                  ).catch(err => this.#errorWhenRoomActive(err, userID))
                }).catch(err => console.log(err))
              break;
